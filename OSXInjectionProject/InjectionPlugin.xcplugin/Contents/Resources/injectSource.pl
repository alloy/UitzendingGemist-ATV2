...
my $newBundle = "$bundleRoot/$productName.bundle";
print "$newBundle\n";

#my $bundleRoot = "/Applications/AppleTV.app/Appliances/Gemist.frappliance";
#my $newBundle = "/Applications/AppleTV.app/Appliances/Gemist.frappliance/$productName.bundle";

0 == system "rm -rf \"$newBundle\" && cp -r \"$bundlePath\" \"$newBundle\""
    or error "Could not copy bundle to: $newBundle";

#$bundlePath = $newBundle;
my $destBundleRoot = "/Applications/AppleTV.app/Appliances/Gemist.frappliance";
$bundlePath = "$destBundleRoot/$productName.bundle";
...
#ssh ${ATV_SSH_USER}@${ATV_ADDRESS} "cd ${ATV_APPLIANCE_INSTALL_PATH}; tar -xvzf ${ATV_APPLIANCE_INSTALL_PATH}/${EXECUTABLE_FOLDER_PATH}.tar.gz; rm ${EXECUTABLE_FOLDER_PATH}.tar.gz; killall -9 AppleTV"

print "Copying to Apple TV...\n";

my $archiveName = "$productName.bundle.tar.gz";
my $deployCommand = "cd $bundleRoot && tar -cvzf '$archiveName' '$productName.bundle' && scp '$archiveName' 'apple-tv.local:$destBundleRoot' && ssh apple-tv.local 'cd $destBundleRoot && tar -xvzf $archiveName && rm $archiveName'";
print "$deployCommand\n";

0 == system $deployCommand
    or error "Could not copy bundle to Apple TV";
