...
//#define INJECTION_IPHONE_IPAD 1
#define INJECTION_ATV 1
#else
#define INJECTION_OSX 1
#endif

#ifdef DEBUG
#define INLog(...) NSLog(@"[INJECTION] %@", [NSString stringWithFormat:__VA_ARGS__])
#else
...
INLog(@"LOAD BUNDLE!");
                        if ( header.dataLength == INJECTION_MAGIC )
                            ...
                            INLog( @"Synchronization error." );
                        if ( !status )
                            INLog( @"*** Bundle has failed to load. If this is due to symbols not found, make sure that Build Setting 'Symbols Hidden by Default' is NO for your Debug build. ***");
...
#ifdef INJECTION_IOS
#ifdef INJECTION_ATV
                        BRImage *img = [[BRImage alloc] initWithData:data];
#else
...
#ifdef INJECTION_ATV
                                NSLog( @"Color objects are not supported on ATV (TODO: Use CGColorRef?)" );
#else
                                float r, g, b, a;
...
+ (void)loadBundle {
    ...
    CFBundleRef bundle = CFBundleCreate(NULL, CFURLCreateWithString(NULL, (__bridge CFStringRef)[NSString stringWithUTF8String:path], NULL));
    if ( !bundle )
    ...
    if (!CFBundleLoadExecutableAndReturnError(bundle, &error)) {
        INLog( @"Load error: %@", (__bridge NSString *)CFErrorCopyFailureReason(error) );
    }
}
